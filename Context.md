...
- **Browser work should be repeatable / semi-deterministic where the slice allows it.** Step 2 should not be purely free-form. The skill provides probe sequences (arm network → locate by visible label → trigger → wait the documented settle window → re-observe → diff against prior state) so the verify/improve/enrich loop is automatable where possible. How procedural a given slice is depends on the slice itself: a required-field validation is highly automatable; a timing-sensitive multi-actor workflow involves more judgment. The trade-off is per slice, not a blanket rule.
- **Don't over-specify the surrounding workflow.** The skill gives framework + judgment cues for slice identification, classification, and prose phrasing. The browser playbook is the most prescriptive part; the rest exercises judgment within the framework.
