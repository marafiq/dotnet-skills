...
**Larger features decompose into multiple linked slice artifacts.** The Care Tracking feature, for example, surfaces as several slices: the shift summary card, the per-task editor, the commit toolbar, the filter bar, the sort segmented controls. Each gets its own focused artifact; they link via `related_controls` (parent / child / sibling / trigger / target) and `scoped_by` for global context. The feature is the graph of artifacts, not one mega-document — small, linked artifacts beat one verbose file for both authoring economy and downstream consumption.

**Expected size.** Most slices land at **150–300 lines** of YAML frontmatter + brief Markdown body. Mutating slices with multiple endpoints, SignalR participation, and full tamper coverage run **~400–650 lines** — that is the upper end, not the norm. The three example artifacts in `examples/` are reference-grade for that upper end; a typical read-only or simple-form slice is much smaller. If your slice is hitting 700+ lines, look for natural decomposition (separate the toolbar from the editor; separate the filter bar from the grid).
...
Generalize observed data values **everywhere in the artifact** — frontmatter, claims, edge-case prose, AND the verification log. The artifact is a behavioral contract, not a data dump; one tenant's data should not pollute it.

**Reject** anywhere in the artifact: tenant names, tenant ids, resident names, room numbers, emails, dates of birth, phone numbers, addresses, hostnames, any identifier that ties the artifact to a single real tenant or person.
...
8. **Mode B unknowns are gate-aware.** Entries in `unknowns_to_fill_when_source_arrives` that mention endpoint paths, anti-forgery, authorization, tenant_boundary scenarios, business_logic.selection.predicates / projection / ordering, or audit emission FORCE `incomplete`. Mode B is a legitimate working mode but cannot ship `complete` while security or correctness fundamentals are deferred to a future source-arrival.
...
10. **`n/a` coherence — no laundering.** `n/a` is a legitimate status only when the aspect provably does not apply. Every `n/a` on a gate-critical field requires an `n/a_reason` that names *why* it doesn't apply. Specifically:
    - `verification.authorization: n/a` and `verification.error_shape: n/a` on **mutating** (`mutates_state: true`) OR **tenant-scoped** endpoints REQUIRE an explicit `contract_status_exceptions` entry with `reason` + `risk_owner` — the default is BLOCKING. The combination "tenant-scoped + authorization n/a" is suspicious by construction and demands the waiver.
    - `verification.anti_forgery: n/a` is acceptable on pure read-only GETs (and most tenant-scoped reads). It REQUIRES a `contract_status_exceptions` entry only on **mutating** endpoints (`mutates_state: true`). This narrowed rule prevents waiver-spam on read-only tenant-scoped GETs while still catching the canonical legacy escape hatch — anti-forgery `n/a` on a GET-shaped write.
...
- [`assets/artifact-template.md`](assets/artifact-template.md) — rich frontmatter schema accommodating all observed behavior types. Currently `schema_version: 7`.
